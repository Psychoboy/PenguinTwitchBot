...
{
    public class ObsSetSceneHandlerTests
    {
        ...
        [Fact]
        public async Task MissingSceneName_ThrowsException()
        {
            var connectionManager = Substitute.For<IOBSConnectionManager>();
            var logger = Substitute.For<ILogger<ObsSetSceneHandler>>();
            var handler = new ObsSetSceneHandler(connectionManager, logger);

            var type = new ObsSetSceneType { OBSConnectionId = 1, SceneName = "" };
            var variables = new ConcurrentDictionary<string, string>();
