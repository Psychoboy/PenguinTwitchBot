...
{
    public class ObsSetSourceMuteStateHandlerTests
    {
        ...
        [Fact]
        public async Task ValidType_MutesSource()
        {
            var connectionManager = Substitute.For<IOBSConnectionManager>();
            var logger = Substitute.For<ILogger<ObsSetSourceMuteStateHandler>>();
            var handler = new ObsSetSourceMuteStateHandler(connectionManager, logger);

            var (connection, mockObs) = CreateConnectedConnection(1, "Main");
            connectionManager.GetManagedConnection(1).Returns(connection);

            var type = new ObsSetSourceMuteStateType { OBSConnectionId = 1, InputName = "Mic", Muted = true };
            var variables = new ConcurrentDictionary<string, string>();

            await handler.ExecuteAsync(type, variables);

            connectionManager.Received(1).GetManagedConnection(1);
            mockObs.Received(1).SetInputMute("Mic", true);
        }
        ...
        public async Task MissingInputName_ThrowsException()
        {
            ...
            var type = new ObsSetSourceMuteStateType { OBSConnectionId = 1, InputName = "", Muted = true };
            var variables = new ConcurrentDictionary<string, string>();
