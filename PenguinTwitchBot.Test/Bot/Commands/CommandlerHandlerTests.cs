...
namespace PenguinTwitchBot.Tests.Bot.Commands
{
    public class CommandHandlerTests
    {
        ...
        public async Task CheckPermission_ShouldFail_ForNonFollower()
        {
            ...
            var viewerFeatureMock = new Mock<PenguinTwitchBot.Bot.Commands.Features.IViewerFeature>();
            viewerFeatureMock.Setup(v => v.IsFollowerByUsername(It.IsAny<string>())).ReturnsAsync(false);

            var scopeMock = new Mock<IServiceScope>();
            scopeMock.Setup(s => s.ServiceProvider.GetService(typeof(PenguinTwitchBot.Bot.Commands.Features.IViewerFeature))).Returns(viewerFeatureMock.Object);
            _scopeFactoryMock.Setup(f => f.CreateScope()).Returns(scopeMock.Object);
