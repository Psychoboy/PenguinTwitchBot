...
namespace DotNetTwitchBot.Bot.Commands.Features
{
    public class ViewerFeature : BaseCommandService, IHostedService, IViewerFeature
    {
        ...
        public ViewerFeature(
            ...
            ) : base(serviceBackbone, commandHandler, "ViewerFeature", dispatcher)
        {
            _logger = logger;
        ...
        public async Task<List<Viewer>> SearchForViewer(string name)
        {
            ...
            await using var scope = _scopeFactory.CreateAsyncScope();
            var db = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            return await db.Viewers.Find(x => x.Username.Contains(normalizedName) || x.DisplayName.Contains(name)).ToListAsync();
        }
        ...
        private async Task OnFollow(object? sender, FollowEventArgs e)
        {
            ...
            await ServiceBackbone.SendChatMessage($"Thank you for following {e.DisplayName} <3", false);
