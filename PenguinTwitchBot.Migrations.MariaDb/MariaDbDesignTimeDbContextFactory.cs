...
namespace PenguinTwitchBot.Migrations.MariaDb;

public sealed class MariaDbDesignTimeDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>
{
	public ApplicationDbContext CreateDbContext(string[] args)
	{
		...
		var connectionString = config.GetConnectionString("MariaDbConnection")
			?? Environment.GetEnvironmentVariable("PenguinTwitchBot_MARIADB_CONNECTION")
			?? "Server=localhost;Database=PenguinTwitchBot;User=root;Password=Password;charset=utf8mb4;";

		var optionsBuilder = new DbContextOptionsBuilder<ApplicationDbContext>();
		optionsBuilder.UseMySql(connectionString, ServerVersion.AutoDetect(connectionString), options =>
		{
			options.MigrationsAssembly(typeof(MariaDbDesignTimeDbContextFactory).Assembly.GetName().Name);
		});
