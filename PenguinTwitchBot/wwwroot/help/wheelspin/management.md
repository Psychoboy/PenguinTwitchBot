...
- **Wheel Name**: An internal identifier for the wheel (also usable in chat commands like `!showwheel <name>`).
- **Winning Message**: The chat announcement sent when a slice is selected (supports the `{WheelSpinResult}` variable).
- **Adding Slices**:
